...
function mainLoop() {
	...
	$.get({
		...
		complete: () => { setTimeout(mainLoop, 5000); },
		success: (json) => {
			...
			if(json.hasOwnProperty("logtime")) {
				...
				console.log(nowDate);
				console.log(dif);
				$("#timestamp").text(json["logtime"]).css("color", (dif > 60) ? "red" : "var(--color-on-main)");
			...
			if(json_has(  "370", "1")) $(  "#370_1").html(round(json_get(  "370", "1") * 1   , 1));
...
$('#btnReboot').on('click', function() {
	if(!confirm("Are you sure you want to REBOOT the EMX?")) return false;
	$.get({
		url: "cmd/reboot.php",
