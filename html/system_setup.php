...
$inverterModel = $dataSettings["Inverter"]["1"]["s1"];
$firmwareVersion = $dataSettings["Inverter"]["3"]["s1"];
$hasGridReconnectDelay = isset($dataSettings["Inverter"]["225"]);
$hasLfsmoDroop = isset($dataSettings["Inverter"]["349"]);
    ...
    <body>
            ...
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <span style="color: red"><b><?php echo $boxType == "livex" ? $lang["system_setup"]["msg_livex_not_registered"] : $lang["system_setup"]["msg_emx_not_registered"]; ?></b></span>
                        <div class="mt-3">
        ...
        <div class="modal fade" id="errorBmsNotExistOrWithOtherSystem" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <span style="color: red"><b><?php echo $lang["system_setup"]["msg_bms_not_exist_or_registered_with_other_system"] ?></b></span>
                        <div class="mt-3">
                            <span class="d-block"><b><?php echo $lang["common"]["serialnumber"] ?></b></span>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <?php if($isTor): ?>
                            ...
                            <?php
                                function getExtendedParameterLimits($dataSettings, $parameterId, $unitMultiplier, $fallbackMin, $fallbackMax, $fallbackStep) {
                                    if (!isset($dataSettings["Inverter"][$parameterId])) {
                                        return [
                                            'min' => $fallbackMin,
                                            'max' => $fallbackMax,
                                            'step' => $fallbackStep
                                    ...
                                    $setting = $dataSettings["Inverter"][$parameterId];
                                    $min = $setting["v1"] * $unitMultiplier;
                                    $max = $setting["v2"] * $unitMultiplier;
                                    $step = $setting["v3"] * $unitMultiplier;
                                    
                                    return [
                                        'min' => rtrim(rtrim(number_format($min, 3, '.', ''), '0'), '.'),
                                        'max' => rtrim(rtrim(number_format($max, 3, '.', ''), '0'), '.'),
                                        'step' => rtrim(rtrim(number_format($step, 3, '.', ''), '0'), '.')
                                    ];
                                }

                                function template_add_extended_parameter($id, $name, $value, $unit, $parameterId, $unitMultiplier, $fallbackStep, $fallbackMin, $fallbackMax) {
                                    global $dataSettings;
                                    $limits = getExtendedParameterLimits($dataSettings, $parameterId, $unitMultiplier, $fallbackMin, $fallbackMax, $fallbackStep);
                                        ...
                                        <div class='form-group row mb-2'>
                                            ...
                                            <div class='col-6 d-flex align-items-center'>
                                                <div class='input-group input-group-sm'>
                                                    <input id='{$id}' class='form-control form-control-outline text-monospace tor-extended-param' type='number' step='{$limits['step']}' min='{$limits['min']}' max='{$limits['max']}' value='{$value}' placeholder='{$value}' title='Range: {$limits['min']} - {$limits['max']}'>
                            ...
                            <?php echo template_add_extended_parameter("extended_fOver1", "f >", "51.5", "Hz", "234", 0.01, "0.01", "50.00", "55.00"); ?>
                            <?php echo template_add_extended_parameter("extended_fUnder1", "f <", "47.5", "Hz", "233", 0.01, "0.01", "45.00", "50.00"); ?>
                            <?php echo template_add_extended_parameter("extended_UeffOver1Time", "Ueff >", "0.1", "sec", "236", 0.001, "0.02", "0", "1000.000"); ?>
                            <?php echo template_add_extended_parameter("extended_UeffUnder1Time", "Ueff <", "1.5", "sec", "235", 0.001, "0.02", "0", "1000.000"); ?>
                            <?php echo template_add_extended_parameter("extended_UeffOver2Time", "Ueff >>", "0.1", "sec", "244", 0.001, "0.02", "0", "1000.000"); ?>
                            <?php echo template_add_extended_parameter("extended_UeffUnder2Time", "Ueff <<", "0.5", "sec", "243", 0.001, "0.02", "0", "1000.000"); ?>
                            ...
                            <?php echo template_add_extended_parameter("extended_puTime", $lang["system_setup"]["extended_putime"], "5", "sec", "336", 0.001, "1", "3", "60"); ?>
                        ...
                        <?php else: ?>
                            <div class="form-group row mb-0">
                                <label class="col-6 col-form-label col-form-label-sm"><?php echo $lang["system_setup"]["extended_gradient"] ?></label>
                                <div class="col-6 d-flex align-items-center">
                                    <div class="input-group input-group-sm">
                                        <input id="extended_lfsmoSlope" class="form-control form-control-outline text-monospace" type="number" step="1" min="10" max="100" value="40" placeholder="40">
        ...
        <div class="modal fade" id="modalConfirmUpsMode" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <h5 class="modal-header mb-0"><?php echo $lang["system_setup"]["upsmodeconfirm_title"] ?></h5>
                    <div class="modal-body">
                        <p><?php echo $lang["system_setup"]["upsmodeconfirm_message"] ?></p>
                        <img class="img-fluid rounded" src="img/clix_ups_mode.png">
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-sm px-4 py-2 btn-success ripple" style="min-width:50%"><b><?php echo $lang["system_setup"]["upsmodeconfirm_confirm"] ?></b></button></div>
                </div>
            </div>
        </div>
        <div class="modal fade" id="modalConfirmBackupMode" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <h5 class="modal-header mb-0"><?php echo $lang["system_setup"]["backupmodeconfirm_title"] ?></h5>
                    <div class="modal-body">
                        <p><?php echo $lang["system_setup"]["backupmodeconfirm_message"] ?></p>
                        <img class="img-fluid rounded" src="img/clix_backup_mode.png">
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-sm px-4 py-2 btn-success ripple" style="min-width:50%"><b><?php echo $lang["system_setup"]["backupmodeconfirm_confirm"] ?></b></button></div>
                </div>
            </div>
        </div>
        <div class="modal fade" id="modalConfirmOtherBatteries" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <h5 class="modal-header mb-0"><?php echo $lang["system_setup"]["otherbatteriesconfirm_title"] ?></h5>
                    <div class="modal-body">
                        <p class="mb-3"><?php echo $lang["system_setup"]["otherbatteriesconfirm_message1"] ?></p>
                ...
                <div class="row">
                    ...
                    <div id="bxHome" class="col-lg-6 col-xl-4 pt-5">
                        ...
                        <div class="main-card card elevate-1 h-100">
                            ...
                            <?php endif; ?>
                            <div id="system_co_box" class="card-body border-bottom" <?php if($boxType == "livex"): ?>style="display: none"<?php endif; ?>>
                            ...
                            <div class="card-body border-bottom pt-3">
                                <label for="bx_box"><?php echo $boxType == "livex" ? $lang["system_setup"]["system_serialnumber_livex"] : $lang["system_setup"]["system_serialnumber_emx"]; ?></label>
                            ...
                            <?php if($boxType == "livex"): ?>
                                <div id="system_mode" class="card-body border-bottom pt-3">
                                    <label for="bx_sysmode"><?php echo $lang["system_setup"]["system_mode"] ?></label>
                                    <select id="bx_sysmode" class="form-control form-control-outline" required>
                                        <option value="0"><?php echo $lang["system_setup"]["system_mode_ups"] ?></option>
                                        <option value="1"><?php echo $lang["system_setup"]["system_mode_backup"] ?></option>
                                    </select>
                                </div>
                            <?php endif; ?>
                    ...
                    <div id="solar" class="col-lg-6 col-xl-4 pt-5">
                        ...
                        <div class="card elevate-1 h-100" style="height: calc(100% - 3.75rem - 4.5rem) !important">
                            ...
                            <?php if($hasSolarWideInputRange): ?>
                            <div id="solar_wideinputrange_container" class="card-body border-bottom pt-3">
...
MPPT 2
    ...
    String 2: ..."></textarea>
                            ...
                            <?php if($isSinglePhase): ?>
                                <div id="box_emeter_phase" class="card-body border-top pt-3">
                            ...
                            <div class="card-body border-top">
                                ...
                                <div id="battery_section_0">
                                    <?php if($boxType != "livex"): ?>
                                        ...
                                        <textarea id="lifepo_bms" class="form-control form-control-outline mb-3" placeholder="xxxxxxxxxxxxxxxx"></textarea>
                                    <?php endif; ?>
                                    <label for="lifepo_serialnumbers"><?php echo $lang["system_setup"]["batteries_lifepo_sn_battery"] ?></label>
        ...
        <script>
            ...
            const isVde4105 = <?php echo $isVde4105 ? "true" : "false"; ?>;
            const isTor = <?php echo $isTor ? "true" : "false"; ?>;
